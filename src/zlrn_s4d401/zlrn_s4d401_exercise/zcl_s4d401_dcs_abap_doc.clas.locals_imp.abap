...
CLASS lcl_passenger_flight DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF st_flights_buffer,
             ...
             plane_type_id  TYPE zlrn_passflight-plane_type_id,
             seats_max      TYPE zlrn_passflight-seats_max,
...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    SELECT
      FROM zlrn_connection AS c
      LEFT OUTER JOIN zlrn_airport AS f
        ON c~airport_from_id = f~airport_id
      LEFT OUTER JOIN zlrn_airport AS t
...
CLASS lcl_cargo_flight DEFINITION INHERITING FROM lcl_flight.
  ...
  PUBLIC SECTION.
    ...
    METHODS
      get_free_capacity
        RETURNING
          VALUE(r_result) TYPE zlrn_plane_actual_load.
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF st_flights_buffer,
             ...
             maximum_load    TYPE zlrn_plane_maximum_load,
             actual_load     TYPE zlrn_plane_actual_load,
             load_unit       TYPE zlrn_plane_weight_unit,
             airport_from_id TYPE /dmo/airport_from_id,
             airport_to_id   TYPE /dmo/airport_to_id,
             departure_time  TYPE /dmo/flight_departure_time,
    ...
    DATA maximum_load TYPE zlrn_plane_maximum_load.
    DATA actual_load TYPE zlrn_plane_actual_load.
    DATA load_unit    TYPE zlrn_plane_weight_unit.
...
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_s4d401_exs_failed | Instantiation failed - evaluate the exception text for details.
    ...
    METHODS find_cargo_flight
      IMPORTING
        ...
        i_airport_to_id   TYPE /dmo/airport_to_id
        i_from_date       TYPE /dmo/flight_date
        i_cargo           TYPE zlrn_plane_actual_load
      EXPORTING
        e_flight          TYPE REF TO lcl_flight
        e_days_later      TYPE i.
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_instance.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_s4d401_exs_failed
        EXPORTING
          textid     = zcx_s4d401_exs_failed=>carrier_not_exist
*         previous   =
          carrier_id = i_carrier_id.
    ...
    AUTHORITY-CHECK
           OBJECT 'ZLRN_CARR'
               ID 'ZLRN_CARR' FIELD i_carrier_id
               ID 'ACTVT'     FIELD '03'.

    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_s4d401_exs_failed
        EXPORTING
          textid     = zcx_s4d401_exs_failed=>carrier_no_read_auth
*         previous   =
          carrier_id = i_carrier_id.
    ENDIF.
