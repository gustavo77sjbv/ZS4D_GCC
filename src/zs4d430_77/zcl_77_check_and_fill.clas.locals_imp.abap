...
CLASS lcl_table IMPLEMENTATION.
  ...
  METHOD copy_with_modevi.
    ...
    LOOP AT <source> ASSIGNING FIELD-SYMBOL(<source_row>).
      ...
      " 2. Map fields with different names in Z77_EMPLOY
      ...
      IF <t_dept> IS ASSIGNED.
        ...
        " Clean and assign
        CONDENSE lv_did.
        <t_dept> = CONV Z77_department_id( lv_did ).
...
*    ENDIF.
  ...
  METHOD fill_depment_manual.
    ...
    DATA lt_departments TYPE TABLE OF Z77_depment.
...
*        assistant_id          = lo_random->get_next( )
 ...
 METHOD assign_managers_to_depts.
  " 1. Fetch data
  SELECT * FROM Z77_employ INTO TABLE @DATA(lt_employees).
  SELECT * FROM Z77_depment INTO TABLE @DATA(lt_departments).
  ...
  MODIFY Z77_depment FROM TABLE @lt_departments.
ENDMETHOD.
...
*             table->copy_with_modevi( ).
  ...
  METHOD run_custom_fill.
    ...
    LOOP AT tables INTO DATA(table).
      ...
      IF table->get_source( ) = 'Z77_DEPMENT'.
        TRY.
